...
# Sökväg till katalogen med .po-filer
base_folder = os.path.expanduser('~/weblatetools/local_po')
po_files = [f for f in os.listdir(base_folder) if f.endswith('.po')]

last_module_path = None
...
for po_file in po_files:
    ...
    try:
        ...
        last_module_path = module_path
    except Exception as e:
        print(f"Fel vid flytt av {po_file}: {e}")

# Efter slutfört arbete, skriv ut sökvägen till sista modulen
if last_module_path:
    print(f"Uppdaterad sökväg:\n{last_module_path}")
else:
    print("Inget projekt att byta till.")
